...
AgentFlow-RS 是一个使用 Rust 编写的高性能、强类型的 AI 智能体工作流引擎。系统立足于“配置即代码”的声明式思想，构建了严谨的**有向无环图 (DAG)** 调度内核，并横向扩展了 **GUI 可视化设计态**、**TUI 极客运行态** 与 **Headless Web 微服务** 三大形态，助力开发者快速构建具有**可视化工作流设计编排**、**ReAct 自主思考**、**本地 RAG 检索**与**多模态文件解析**能力的前沿 AI 原生应用。

## 1. ✨ 核心特性

- ⚡ **无锁化异步并发调度**：基于入度表与 Kahn 拓扑排序算法，结合 `tokio` 与 `Arc<RwLock<T>>`，实现图节点的安全、最大化并发执行。
- ⚙️ **声明式 YAML 驱动**：全面贯彻“配置即代码 (IaC)”理念，工作流拓扑与节点属性完全由直观的 YAML 文件定义。支持动态加载、高度复用与 Git 版本控制，实现业务逻辑与底层引擎的极致解耦。
- 🛠️ **全能 CLI 命令行入口**：基于 clap 构建的高级终端命令解析器。只需一行命令与不同参数，即可无缝切换 GUI 设计、TUI 监控或微服务挂起等多种运行形态，并支持通过命令行传入 Debug 遥测等底层指令。
